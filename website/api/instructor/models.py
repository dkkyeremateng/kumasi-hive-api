...
class Instructor(db.Document):
    ...
    def find_by_status(cls, status=True):
        ...
        """
        return Instructor.objects.filter(is_active=status)
    ...
    def find_by_external_id(cls, external_id, is_active=True):
        ...
        :return: Instructor instance
        """
        return Instructor.objects.filter(external_id=external_id, is_active=is_active).first()
